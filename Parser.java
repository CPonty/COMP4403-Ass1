...
/******************************* Constants **********************************/
    ...
    private StatementNode.AssignmentNode parseAssignment(TokenSet recoverSet) {
         ...
         * *Added COMMA to the set for similar reasons
         */
        TokenSet recoverLVal = recoverSet.union( Token.ASSIGN, Token.EQUALS, Token.COMMA );
        TokenSet recoverRVal = recoverSet.union( Token.COMMA );
        // should match at least one assignee
        ArrayList<ExpNode> leftList = new ArrayList<ExpNode>();
        ArrayList<ExpNode> rightList = new ArrayList<ExpNode>();
        Position pos = token.getPosn();
        leftList.add( parseLValue( recoverLVal ) );
        while( token.isMatch(Token.COMMA) ) {
            match( Token.COMMA, LVALUE_START_SET );
            pos = token.getPosn();
            leftList.add( parseLValue( recoverLVal ) );
        }
        match( Token.ASSIGN, CONDITION_START_SET );
        // should match at least one condition
        rightList.add( parseCondition( recoverRVal ) );
        while( token.isMatch(Token.COMMA) ) {
            match( Token.COMMA, CONDITION_START_SET );
            ExpNode right = parseCondition( recoverRVal );
            // ensure we don't have more right-hand conditions than assignees
            if( leftList.size() == rightList.size() ) {
                error("too many expressions in multiple assignment", token.getPosn());
            } else {
                rightList.add( right );
            }
        }
        // ensure all assignments have matching right sides
        if( leftList.size() > rightList.size() ) {
            error("not enough expressions in multiple assignment", pos);
            while( leftList.size() > rightList.size() ) {
                rightList.add( new ExpNode.ErrorNode( token.getPosn() ) );
            }
        }
        endRule( "Assignment", recoverSet );
        return new StatementNode.AssignmentNode(pos, leftList, rightList);
    }
    ...
    /** Rule: ForStatement -> KW_FOR IDENTIFIER COLON LBRACKET Condition RANGE Condition RBRACKET KW_DO Statement */
    private StatementNode parseForStatement( TokenSet recoverSet ) {
        ...
        match( Token.IDENTIFIER, recoverSet.union(Token.COLON) ); /* declare later - static checking */
        match( Token.COLON, recoverSet.union(Token.LBRACKET) );
        match( Token.LBRACKET, CONDITION_START_SET );
        ExpNode startCondition = parseCondition( recoverSet.union(Token.RANGE) );
        match( Token.RANGE, CONDITION_START_SET );
        ExpNode endCondition = parseCondition( recoverSet.union(Token.RBRACKET) );
        ...
        StatementNode loopStatement = this.parseStatement( recoverSet );
        symtab.leaveExtendedScope();
        endRule( "For Statement", recoverSet );
        
        return new StatementNode.ForNode(pos, controlVar, startCondition, 
                endCondition, loopStatement, localScope);
    }
